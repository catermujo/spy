...
log_allocator :: proc(la: ^Log_Allocator) -> runtime.Allocator {
    return {procedure = log_allocator_proc, data = la}
}
...
log_allocator_proc :: proc(
    ...
    case .Alloc:
        ...
        switch la.size_fmt {
        case .Bytes:
            format = "%s%s>>> ALLOCATOR(mode=.Alloc, size=%d, alignment=%d)"
        case .Human:
            format = "%s%s>>> ALLOCATOR(mode=.Alloc, size=%m, alignment=%d)"
        }
    ...
    case .Alloc_Non_Zeroed:
        ...
        switch la.size_fmt {
        case .Bytes:
            format = "%s%s>>> ALLOCATOR(mode=.Alloc_Non_Zeroed, size=%d, alignment=%d)"
        case .Human:
            format = "%s%s>>> ALLOCATOR(mode=.Alloc_Non_Zeroed, size=%m, alignment=%d)"
        }
        str := fmt.bprintf(buf[:], format, la.prefix, padding, size, alignment)
        context.logger.procedure(context.logger.data, la.level, str, context.logger.options, location)
    ...
    case .Free:
        if old_size != 0 {
            ...
            switch la.size_fmt {
            case .Bytes:
                format = "%s%s<<< ALLOCATOR(mode=.Free, ptr=%p, size=%d)"
            case .Human:
                format = "%s%s<<< ALLOCATOR(mode=.Free, ptr=%p, size=%m)"
            }
    ...
    case .Resize:
        ...
        switch la.size_fmt {
        case .Bytes:
            format = "%s%s>>> ALLOCATOR(mode=.Resize, ptr=%p, old_size=%d, size=%d, alignment=%d)"
        case .Human:
            format = "%s%s>>> ALLOCATOR(mode=.Resize, ptr=%p, old_size=%m, size=%m, alignment=%d)"
        }
    ...
    case .Resize_Non_Zeroed:
        ...
        switch la.size_fmt {
        case .Bytes:
            format = "%s%s>>> ALLOCATOR(mode=.Resize_Non_Zeroed, ptr=%p, old_size=%d, size=%d, alignment=%d)"
        case .Human:
            format = "%s%s>>> ALLOCATOR(mode=.Resize_Non_Zeroed, ptr=%p, old_size=%m, size=%m, alignment=%d)"
        }
        str := fmt.bprintf(buf[:], format, la.prefix, padding, old_memory, old_size, size, alignment)
        context.logger.procedure(context.logger.data, la.level, str, context.logger.options, location)
